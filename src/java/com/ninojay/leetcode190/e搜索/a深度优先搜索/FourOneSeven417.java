package com.ninojay.leetcode190.e搜索.a深度优先搜索;
...
public class FourOneSeven417 {
    ...
    public static void main(String[] args) {
        int[][] heights = NinoCreationUtil.createTwoDimensionArray("" +
                ...
                "[6,7,1,4,5]," +
                "[5,1,1,2,4]]");
        List<List<Integer>> lists = new FourOneSeven417().pacificAtlantic(heights);
    ...
    private void dfs(int[][] heights, boolean[][] flag, int i, int j) {
        if(flag[i][j]) {
            return;
        }
        flag[i][j] = true;
